...
//      tep_draw_hidden_field('a_languages_check[' . $i . ']', 'set');
    ...
    echo '<td class="main">' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . ' ' . tep_draw_input_field('headlines[' . $a_languages_id . ']', $affiliate_news['headline'], '', true) . '</td>'; ?>
        ...
        <td class="main" align="right">
          <?php
            isset($_GET['affiliate_news_id']) ? $cancel_button = '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_AFFILIATE_NEWS, 'affiliate_news_id=' . $_GET['affiliate_news_id']) . (isset($_GET['lngdir']) ? '&lngdir=' . $_GET['lngdir']:'') . '">' . tep_draw_button(IMAGE_CANCEL, 'disk', null, 'primary') . '</a>' : $cancel_button = '';
            echo (isset($_GET['affiliate_news_id']) ? tep_draw_button(IMAGE_UPDATE, 'disk', null, 'primary') : tep_draw_button(IMAGE_INSERT, 'disk', null, 'primary') )  . $cancel_button ;
          ?>
        </td>
          </tr>
          <tr>
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php echo '&nbsp;<a href="' . tep_href_link(FILENAME_AFFILIATE_NEWS, 'action=new_affiliate_news'  . (isset($_GET['lngdir']) ? '&lngdir=' . $_GET['lngdir']:'')) . '">' . tep_draw_button(IMAGE_NEW_NEWS_ITEM, 'disk', null, 'primary') . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
    ...
    switch ($_GET['action']) {
      case 'delete_affiliate_news': //generate box for confirming a news article deletion
        $heading[] = array('text'   => '<strong>' . TEXT_INFO_HEADING_DELETE_ITEM . '</strong>');
        ...
        $contents[] = array('text'  => '<br /><strong>' . $selected_item['headline'] . '</strong>');
        
        $contents[] = array('align' => 'center',
                            'text'  => '<br />' . tep_draw_submit(IMAGE_DELETE, 'disk', null, 'primary') . ' <a href="' . tep_href_link(FILENAME_AFFILIATE_NEWS, 'affiliate_news_id=' . $selected_item['news_id']) . (isset($_GET['lngdir']) ? '&lngdir=' . $_GET['lngdir']:'') . '">' . tep_draw_button(IMAGE_CANCEL, 'disk', null, 'primary') . '</a>');
        break;

      default:
        if ($rows > 0) {
          if (is_array($selected_item)) { //an item is selected, so make the side box
            $heading[] = array('text' => '<strong>' . $selected_item['headline'] . '</strong>');

            $contents[] = array('align' => 'center', 
                                'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE_NEWS, 'affiliate_news_id=' . $selected_item['news_id'] . '&action=new_affiliate_news' . (isset($_GET['lngdir']) ? '&lngdir=' . $_GET['lngdir']:'') ) . '">' . tep_draw_button(IMAGE_EDIT, 'disk', null, 'primary') . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE_NEWS, 'affiliate_news_id=' . $selected_item['news_id'] . '&action=delete_affiliate_news' . (isset($_GET['lngdir']) ? '&lngdir=' . $_GET['lngdir']:'')) . '">' . tep_draw_button(IMAGE_DELETE, 'disk', null, 'primary') . '</a>');
