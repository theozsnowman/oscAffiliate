...
?>
      ...
      <tr>
       ...
       <tr>
        <td align="right" class="main"><?php echo tep_draw_button(IMAGE_UPDATE, 'disk', null, 'primary') . ' <a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('action'))) .'">' . tep_draw_button(IMAGE_CANCEL, 'disk', null, 'primary') . '</a>';?></td>
      </tr></form>
<?php
  } else {
?>
...
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE) . '">' . tep_draw_button(IMAGE_RESET, 'disk', null, 'primary') . '</a>'; ?></td>
                  </tr>
<?php
    }
?>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'confirm':
      ...
      $contents[] = array('text' => TEXT_DELETE_INTRO . '<br /><br /><strong>' . $aInfo->affiliate_firstname . ' ' . $aInfo->affiliate_lastname . '</strong>');
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'disk', null, 'primary') . ' <a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID', 'action')) . 'acID=' . $aInfo->affiliate_id) . '">' . tep_draw_button(IMAGE_CANCEL, 'disk', null, 'primary') . '</a>');
      break;
    default:
      if (is_object($aInfo)) {
        $heading[] = array('text' => '<strong>' . $aInfo->affiliate_firstname . ' ' . $aInfo->affiliate_lastname . '</strong>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID', 'action')) . 'acID=' . $aInfo->affiliate_id . '&action=edit') . '">' . tep_draw_button(IMAGE_EDIT, 'disk', null, 'primary') . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE, tep_get_all_get_params(array('acID', 'action')) . 'acID=' . $aInfo->affiliate_id . '&action=confirm') . '">' . tep_draw_button(IMAGE_DELETE, 'disk', null, 'primary') . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE_CONTACT, 'selected_box=affiliate&affiliate=' . $aInfo->affiliate_email_address) . '">' . tep_draw_button(IMAGE_EMAIL, 'disk', null, 'primary') . '</a>');
